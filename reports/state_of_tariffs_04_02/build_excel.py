"""
Build data_download.xlsx for State of Tariffs: April 2, 2026.

Reads model output CSVs and creates formatted Excel workbook with all
tables and figure data.

Usage: python reports/state_of_tariffs_04_02/build_excel.py
"""
...
TEMP_DIR = os.path.join(REPO, 'output', '2026-04-02', 'results')
PERM_DIR = os.path.join(REPO, 'output', '2026-04-02_s122perm', 'results')
EXAMPLE_XLSX = os.path.join(REPO, 'reports', 'state_of_tariffs_example', 'data_download.xlsx')
OUT_PATH = os.path.join(REPO, 'reports', 'state_of_tariffs_04_02', 'data_download_v2.xlsx')
...
# Historical ETR data from example file
hist_etr_path = os.path.join(REPO, 'reports', 'state_of_tariffs_example', 'data_download.xlsx')
...
ws = wb.active
ws.title = 'Data TOC'
ws['A1'] = 'State of U.S. Tariffs: April 2, 2026'
...
example_wb = load_workbook(hist_etr_path, data_only=True)
...
# Copy historical rows from example (rows 6 onward)
# The projected/current lines will be updated with new values
new_postsub = temp['pe_postsub_all_in_etr']  # post-sub all-in level for S122 expires
new_presub = temp['pre_sub_all_in_etr']       # pre-sub all-in level for S122 expires
...
for src_row in range(6, example_f1.max_row + 1):
    ...
    # Horizontal reference lines (projected rate after S122 expires)
    ws.cell(row=dest_row, column=3, value=new_postsub)
    ws.cell(row=dest_row, column=5, value=new_presub)

    # Current rate (only for 2024-2025 which show current policy in effect)
    if year_val == 2024:
...
for i, (tr, pr) in enumerate(zip(temp_macro, perm_macro)):
    ...
    yr = int(tr['year'])
    qtr = int(tr['quarter'])
    # Compute GDP % change from baseline
...
# Year headers (skip 2025, show 2026-2035 + total)
